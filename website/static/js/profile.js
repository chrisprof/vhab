...
function updateRender() {
  if (localStorage.getItem("days")) {
    ...
    if (localStorage.getItem("today") != day) {
      doselist.push(
        (doselist[doselist.length - 1] / 2) ^
          (localStorage.getItem("days") / multiplier)
      );
      localStorage.setItem(
        "days",
        Number(localStorage.getItem("days")) +
          (day - localStorage.getItem("today"))
      );
...
let percentageDecrease =
  Math.floor((doselist[doselist.length - 1] / doselist[0]) * 100) + "%";
